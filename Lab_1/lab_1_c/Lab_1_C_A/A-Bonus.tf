...
locals {
  ...
  dawgs_prefix = var.dawgs_project

  # TODO: Students should lock this down after apply using the real secret ARN from outputs/state
  dawgs_secret_arn_guess = "arn:aws:secretsmanager:${data.aws_region.dawgs_region01.region}:${data.aws_caller_identity.dawgs_self01.account_id}:secret:${local.dawgs_prefix}/rds/mysql3*"
}
...
resource "aws_instance" "dawgs_ec201_private_bonus" {
  ...
  tags = {
    Name = "${local.dawgs_prefix}-ec201-private"
  }
}
...
resource "aws_security_group" "dawgs_vpce_sg01" {
  name        = "${local.dawgs_prefix}-vpce-sg01"
...
resource "aws_vpc_endpoint" "dawgs_vpce_s3_gw01" {
  vpc_id            = aws_vpc.dawgs_vpc01.id
  service_name      = "com.amazonaws.${data.aws_region.dawgs_region01.region}.s3"
  ...
  tags = {
    Name = "${local.dawgs_prefix}-vpce-s3-gw01"
  }
}
...
resource "aws_vpc_endpoint" "dawgs_vpce_ssm01" {
  vpc_id              = aws_vpc.dawgs_vpc01.id
  service_name        = "com.amazonaws.${data.aws_region.dawgs_region01.region}.ssm"
  ...
  tags = {
    Name = "${local.dawgs_prefix}-vpce-ssm01"
  }
}
...
resource "aws_vpc_endpoint" "chewbacca_vpce_ec2messages01" {
  vpc_id              = aws_vpc.dawgs_vpc01.id
  service_name        = "com.amazonaws.${data.aws_region.dawgs_region01.region}.ec2messages"
  vpc_endpoint_type   = "Interface"
  private_dns_enabled = true
  ...
  tags = {
    Name = "${local.dawgs_prefix}-vpce-ec2messages01"
  }
}
...
resource "aws_vpc_endpoint" "dawgs_vpce_ssmmessages01" {
  vpc_id              = aws_vpc.dawgs_vpc01.id
  service_name        = "com.amazonaws.${data.aws_region.dawgs_region01.region}.ssmmessages"
  vpc_endpoint_type   = "Interface"
  private_dns_enabled = true
  ...
  tags = {
    Name = "${local.dawgs_prefix}-vpce-ssmmessages01"
  }
}
...
resource "aws_vpc_endpoint" "dawgs_vpce_logs01" {
  vpc_id              = aws_vpc.dawgs_vpc01.id
  service_name        = "com.amazonaws.${data.aws_region.dawgs_region01.region}.logs"
  vpc_endpoint_type   = "Interface"
  private_dns_enabled = true
  ...
  tags = {
    Name = "${local.dawgs_prefix}-vpce-logs01"
  }
}
...
resource "aws_vpc_endpoint" "dawgs_vpce_secrets01" {
  vpc_id              = aws_vpc.dawgs_vpc01.id
  service_name        = "com.amazonaws.${data.aws_region.dawgs_region01.region}.secretsmanager"
  vpc_endpoint_type   = "Interface"
  private_dns_enabled = true
  ...
  tags = {
    Name = "${local.dawgs_prefix}-vpce-secrets01"
  }
}
...
resource "aws_vpc_endpoint" "dawgs_vpce_kms01" {
  vpc_id              = aws_vpc.dawgs_vpc01.id
  service_name        = "com.amazonaws.${data.aws_region.dawgs_region01.region}.kms"
  vpc_endpoint_type   = "Interface"
  private_dns_enabled = true
  ...
  tags = {
    Name = "${local.dawgs_prefix}-vpce-kms01"
  }
}
...
resource "aws_iam_policy" "dawgs_leastpriv_read_params01" {
  name        = "${local.dawgs_prefix}-lp-ssm-read01"
...
resource "aws_iam_policy" "dawgs_leastpriv_read_secret01" {
  name        = "${local.dawgs_prefix}-lp-secrets-read01"
    ...
    Statement = [
      {
        ...
        Resource = local.dawgs_secret_arn_guess
      }
    ]
  })
}
...
resource "aws_iam_policy" "dawgs_leastpriv_cwlogs01" {
  name        = "${local.dawgs_prefix}-lp-cwlogs01"
