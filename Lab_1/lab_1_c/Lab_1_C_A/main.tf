############################################
# Locals (naming convention: dawgs-*)
############################################
locals {
...
# Explanation: dawgs needs a hyperlane—this VPC is the Millennium Falcon’s flight corridor.
resource "aws_vpc" "dawgs_vpc01" {
  cidr_block           = var.dawgs_cidr
...
resource "aws_subnet" "dawgs_private_subnets" {
  count             = length(var.dawgs_private)
  vpc_id            = aws_vpc.dawgs_vpc01.id
  cidr_block        = var.dawgs_private[count.index]
  availability_zone = var.azs[count.index]
...
# Explanation: dawgs wants the private base to call home—EIP gives the NAT a stable “holonet address.”
...
# Explanation: NAT is dawgs’s smuggler tunnel—private subnets can reach out without being seen.
...
# Explanation: Private subnets route outbound internet via NAT (dawgs-approved stealth).
...
# Explanation: EC2 SG is dawgs’s bodyguard—only let in what you mean to.
resource "aws_security_group" "dawgs_ec2_sg01" {
...
resource "aws_security_group" "dawgs_rds_sg01" {
  ...
  ingress {
    ...
    to_port         = 3306
    protocol        = "tcp"
    security_groups = [aws_security_group.dawgs_ec2_sg01.id]
  }
  ...
  # TODO: student adds inbound MySQL 3306 from aws_security_group.dawgs_ec2_sg01.id
...
resource "aws_db_subnet_group" "dawgs_rds_subnet_group01" {
...
resource "aws_db_instance" "dawgs_rds01" {
  ...
  db_subnet_group_name   = aws_db_subnet_group.dawgs_rds_subnet_group01.name
  vpc_security_group_ids = [aws_security_group.dawgs_rds_sg01.id]
...
# Explanation: dawgs refuses to carry static keys—this role lets EC2 assume permissions safely.
...
resource "aws_instance" "dawgs_ec201" {
  ami                    = var.ec2_ami_id
  instance_type          = var.ec2_instance_type
  subnet_id              = aws_subnet.dawgs_public_subnets[0].id
...
# Explanation: Parameter Store is dawgs’ map—endpoints and config live here for fast recovery.
...
resource "aws_secretsmanager_secret" "dawgs_db_secret01" {
  name                    = "${local.name_prefix}/rds/mysql4"
...
resource "aws_cloudwatch_log_group" "dawgs_log_group01" {
...
# Explanation: Metrics are dawgs’s growls—when they spike, something is wrong.
# NOTE: Students must emit the metric from app/agent; this just declares the alarm.
resource "aws_cloudwatch_metric_alarm" "dawgs_db_alarm01" {
  ...
  alarm_actions = [aws_sns_topic.dawgs_sns_topic01.arn]
...
resource "aws_sns_topic" "dawgs_sns_topic01" {
...
resource "aws_sns_topic_subscription" "dawgs_sns_sub01" {
  topic_arn = aws_sns_topic.dawgs_sns_topic01.arn
  protocol  = "email"
...
# resource "aws_vpc_endpoint" "dawgs_vpce_ssm" { ... }