...
resource "aws_cloudwatch_metric_alarm" "dawgs_alb_5xx_alarm01" {
  ...
  dimensions = {
    LoadBalancer = aws_lb.dawgs_alb01.arn_suffix
  }
...
resource "aws_cloudwatch_dashboard" "dawgs_dashboard01" {
    ...
    widgets = [
      {
        ...
        properties = {
          metrics = [
            ["AWS/ApplicationELB", "RequestCount", "LoadBalancer", aws_lb.dawgs_alb01.arn_suffix],
            [".", "HTTPCode_ELB_5XX_Count", ".", aws_lb.dawgs_alb01.arn_suffix]
          ]
          period = 300
          stat   = "Sum"
          region = var.aws_region
          title  = "dawgs ALB: Requests + 5XX"
        }
      },
      {
        type   = "metric"
        ...
        properties = {
          metrics = [
            ["AWS/ApplicationELB", "TargetResponseTime", "LoadBalancer", aws_lb.dawgs_alb01.arn_suffix]
          ]
          period = 300
          stat   = "Average"
          region = var.aws_region
          title  = "dawgs ALB: Target Response Time"
        }
      }
    ]
  })
